...
template <class T>
int List<T>::getFirstPos()
{
    return isEmpty() ? -1 : 0;
}

template <class T>
int List<T>::getLastPos()
{
    return last;
}
...
template <class T>
void List<T>::quickSort(const int& leftEdge, const int& rightEdge, int comp(const T&, const T&))
{
    T aux;
    ...
    while (i < j)
    {
        while (i < j && (comp(*data[i], *data[rightEdge]) == 0 || comp(*data[i], *data[rightEdge]) == -1))
        {
            i++;
        }

        while (i < j && (comp(*data[j], *data[rightEdge]) == 0 || comp(*data[j], *data[rightEdge]) == 1))
        {
            j--;
        }
...
void List<T>::writeToDisk(const string &fileName)
{
    ...
    if (!myFile.is_open())
    {
        throw ListException("No se pudo abrir el archivo para escritura, writeToDisk");
    }
...
void List<T>::readFromDisk(const string& fileName)
{
    ...
    if (!myFile.is_open())
    {
        throw ListException("No se pudo abrir el archivo para lectura, readFromDisk");
    }
